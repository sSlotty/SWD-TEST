...
class StudentSubjectsScoreAPIView(APIView):
    ...
    def post(request, *args, **kwargs):
       ...
       #check type of student_first_name and student_last_name for string
        if type(student_first_name) != str or type(student_last_name) != str or type(subjects_title) != str:
            return Response(status=status.HTTP_400_BAD_REQUEST, data={"message": "first_name, last_name and subject_title must be string"})
        #check type of score for int or float
        if type(score) != int:
            return Response(status=status.HTTP_400_BAD_REQUEST, data={"message": "score must be number"})

        #check score is equal or greater than 0 and equal or less than 100
        if score < 0 or score > 100:
            return Response(status=status.HTTP_400_BAD_REQUEST, data={"message": "score must be equal or greater than 0 and equal or less than 100"})
        
        
        #check student's score of each subject must be unique
        student = Personnel.objects.filter(first_name=student_first_name, last_name=student_last_name)
        if not student.exists():
            return Response(status=status.HTTP_400_BAD_REQUEST, data={"message": "student not found"})
        
        subjects = Subjects.objects.filter(title=subjects_title)
        if not subjects.exists():
            return Response(status=status.HTTP_400_BAD_REQUEST, data={"message": "subject not found"})
        
        studentID = student[0].id
        subjectID = subjects[0].id
        credits = 0
        
        for i in credits_context:
            if subjectID in i["subject_id_list_that_using_this_credit"]:
                credits = i["credit"]
        ...
        student_subjects_score = StudentSubjectsScore.objects.filter(student_id=studentID,subjects_id=subjectID).values()
        if student_subjects_score.exists():
            student_subjects_score = StudentSubjectsScore.objects.filter(student_id=studentID,subjects_id=subjectID)
            student_subjects_score.update(score=score)
            return Response(status=status.HTTP_201_CREATED, data=context_data)
        
        StudentSubjectsScore.objects.create(student_id=studentID,subjects_id=subjectID,score=score,credit=credits)
        return Response(status=status.HTTP_201_CREATED, data=context_data)
...
class StudentSubjectsScoreDetailsAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        student = Personnel.objects.filter(id=student_id).values()
        
        
        if not student.exists():
            return Response(status=status.HTTP_400_BAD_REQUEST, data={"message": "student not found"})
        
        student_subjects_score = StudentSubjectsScore.objects.filter(student_id=student_id).values()
        if not student_subjects_score.exists():
            return Response(status=status.HTTP_400_BAD_REQUEST, data={"message": "student not found"})
        
        
        full_name = student[0]['first_name'] + " " + student[0]['last_name']
        school = Schools.objects.filter(id=student[0]['school_class_id']).values()
        school_name = school[0]['title']
        
        for i in student_subjects_score:
            if i['score'] >= 80:
                i['grade'] = "A"
            elif i['score'] >= 75:
                i['grade'] = "B+"
        ...
        score = 0
        for k in student_subjects_score:
            
            
            for i in subjects_context:
                if k['subjects_id'] == i['id']:
                    k['subject'] = i['title']
                    break
        ...
        average = [x['score'] for x in student_subjects_score] 
        ...
        example_context_data = {
            ...
            "subject_detail": student_subjects_score,

            "grade_point_average": sum(average)/len(average),
        }
...
class PersonnelDetailsAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        print(school_title)
        schools = Schools.objects.filter(title=school_title).values()
        if schools.exists():
            
            schoolsID = schools[0]['id']
            all_personnel = Personnel.objects.filter(school_class_id=schoolsID).values()
            
            data = []
            
            for i in range(len(all_personnel)):
                role = ""
                text = ""
                if all_personnel[i]['personnel_type'] == 0:
                    role = 'Teacher'
                elif all_personnel[i]['personnel_type'] == 1:
                    role = 'Head of the room'
                else:
                    role = 'Student'
                text += str(i+1) + ". school: " + str(school_title).upper() + ", role: " + role + ", class: " + str(all_personnel[i]['school_class_id']) + ", name: " + str(all_personnel[i]['first_name']).upper() + " " + str(all_personnel[i]['last_name']).upper()
                data.append(text)
...
class SchoolHierarchyAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        personnel = Personnel.objects.all().values()
        classes = Classes.objects.all().values()
        
        data = []
        for i in range(len(school)):
            data.append({"school" : school[i]['title']})
            for j in classes:
                if school[i]['id'] == j['school_id']:
                    classText = "class " + str(j['class_order'])
                    data[i][classText] = {}
                    for k in personnel:
                        if j['class_order'] == k['school_class_id']:
                            role = ROLES.get(k['personnel_type'])
                            
                            if role == "Teacher":
                                teacher = role + ": " + k['first_name'] + " " + k['last_name']
                                data[i][classText][teacher] = []
                            else:
                                data[i][classText][teacher].append({role : k['first_name'] + " " + k['last_name']})
...
class SchoolStructureAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        seconds = []
        heights = []
        school_structure = SchoolStructure.objects.all().values()
        for i in school_structure:
            if i['parent_id'] == None:
                data.append({"title": i['title'], "sub": []})
        
        for x in school_structure:
            if x['parent_id'] == 1:
                seconds.append({"title": x['title'], "sub": []})
            if x['parent_id'] == 26:
                heights.append({"title": x['title'], "sub": []})
        
        for i in school_structure:
            if i['parent_id'] == 2:
                seconds[0]['sub'].append({"title": i['title']})
            if i['parent_id'] == 10:
                seconds[1]['sub'].append({"title": i['title']})
            if i['parent_id'] == 18:
                seconds[2]['sub'].append({"title": i['title']})
            if i['parent_id'] == 27:
                heights[0]['sub'].append({"title": i['title']})
            if i['parent_id'] == 35:
                heights[1]['sub'].append({"title": i['title']})
            if i['parent_id'] == 43:
                heights[2]['sub'].append({"title": i['title']})
